...
In this project, we modelled a family of knowing-how logics and implemented model checkers for both $\mathcal{L}_{Kh}$ and $reg\text{-}\mathcal{L}^U_{Kh}$. 
The checker for $\mathcal{L}_{Kh}$ is an explicit procedure with bounded plan length, 
while the checker for $reg\text{-}\mathcal{L}^U_{Kh}$ follows the algorithms described in \cite{Demri2023}. 
In addition, we implemented parsers for both logics and used QuickCheck to test several basic properties of the resulting systems. 
Together, these form the main contributions of the project. Additionally, for demo and testing purpose, 
we've implemented a simple web-application to support model-checking by offering a visual experience and keeping a history of formulas and models (within a session). 