...
First, we test the \textbf{WSKh} principle, which captures a strengthened form of monotonicity. Intuitively, knowing how is stable under \emph{strengthening the precondition} and \emph{weakening the goal}: if \(\psi'\) universally implies \(\psi\), and \(\varphi\) universally implies \(\varphi'\), then any plan that achieves \(\varphi\) from \(\psi\) should also achieve \(\varphi'\) from the more restrictive precondition \(\psi'\).

Second, we test \textbf{compositionality} of knowing how. If an agent knows how to achieve \(\varphi\) from \(\psi\), and also knows how to achieve \(\chi\) from \(\varphi\), then these abilities should compose, yielding a strategy that achieves \(\chi\) from \(\psi\). This property captures the idea that knowing-how is closed under sequential composition of plans.

Third, we test the case of an \textbf{impossible precondition}. If the precondition is contradictory, then no states satisfy it, and both semantic conditions of \(Kh_i(\psi,\varphi)\) are trivially satisfied. Hence, \(Kh_i(\psi,\varphi)\) should hold vacuously in this case.

Fourth, as in the single-agent case, we verify that \(Kh_i(\psi,\varphi)\) is \textbf{global} in the model: its truth value does not depend on the evaluation state, since its semantics quantifies over all states satisfying the precondition.
...
main = hspec $ do
  ...
  describe "Multi-Agent Logic reg-L^U_KH" $ do
    ...
    it "WSKh-style: strengthening the precondition and weakening the goal preserves knowing how" $
      property $ \(PRM (m, s)) agent ->
        let nA = numAgentsIn m
        in forAll (sized (generateRegForm nA)) $ \psi' ->
           forAll (sized (generateRegForm nA)) $ \psi ->
           forAll (sized (generateRegForm nA)) $ \phi ->
           forAll (sized (generateRegForm nA)) $ \phi' ->
             let aIdx = (agent `mod` nA) + 1
             in ( isUniversallyImpliedReg m psi' psi
                  && isUniversallyImpliedReg m phi phi'
                  && isTrueReg (m, s) (KHI aIdx psi phi) )
                ==> isTrueReg (m, s) (KHI aIdx psi' phi')

    it "COMPKh-style: compositionality of knowing how" $
      property $ \(PRM (m, s)) agent ->
        let nA = numAgentsIn m
        in forAll (sized (generateRegForm nA)) $ \psi ->
           forAll (sized (generateRegForm nA)) $ \phi ->
           forAll (sized (generateRegForm nA)) $ \chi ->
             let aIdx = (agent `mod` nA) + 1
             in ( isTrueReg (m, s) (KHI aIdx psi phi)
                  && isTrueReg (m, s) (KHI aIdx phi chi) )
                ==> isTrueReg (m, s) (KHI aIdx psi chi)
    ...
    it "KHI is global: agent ability is a model-wide property" $
    ...
    it "Intersection checking is symmetric: L(A1) ∩ L(A2) is non-empty iff L(A2) ∩ L(A1) is non-empty" $
...
Single-Agent Logic L_Kh
  Boolean: Double Negation Elimination (!!f <=> f) [v]
    +++ OK, passed 100 tests.
  Boolean: Commutativity of Conjunction (f1 ^ f2 <=> f2 ^ f1) [v]
    +++ OK, passed 100 tests.
  Boolean: Identity of Conjunction (f ^ T <=> f) [v]
    +++ OK, passed 100 tests.
  KH is global: truth does not vary between states [v]
    +++ OK, passed 100 tests.
Multi-Agent Logic reg-L^U_KH
  Goal Monotonicity: if phi implies chi, then knowing how to achieve phi implies knowing how to achieve chi [v]       
    +++ OK, passed 100 tests; 152 discarded.
  WSKh-style: strengthening the precondition and weakening the goal preserves knowing how [v]
    +++ OK, passed 100 tests; 377 discarded.
  COMPKh-style: compositionality of knowing how [v]
    +++ OK, passed 100 tests; 169 discarded.
  Vacuous Precondition: knowing how from contradiction holds vacuously [v]
    +++ OK, passed 100 tests.
  KHI is global: agent ability is a model-wide property [v]
    +++ OK, passed 100 tests.
  Intersection checking is symmetric: L(A1) ∩ L(A2) is non-empty iff L(A2) ∩ L(A1) is non-empty [v]
    +++ OK, passed 100 tests.

Finished in 218.8784 seconds
